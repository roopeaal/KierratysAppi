...
Basic use requires no account. Camera frames are decoded locally and not uploaded. Typed material-code input is capped at 64 characters, normalized by a deterministic allowlist parser, and remains on-device. Optional history and correction drafts use AsyncStorage and start local-only; disabling history clears it. Technical telemetry is no-op until a vendor/consent decision and has a tested forbidden-property boundary for GTIN/barcode/product name/photo/location/free text. Location is not requested.
